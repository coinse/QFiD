...
if [ -d "$TEST_SUITE_CONTENT" ]; then
  ...
  for class in $(cat $RELEVANT_CLASSES); do
    ...
    if [ -f "$RELEVANT_METHODS_DIR/$class" ]; then
      ...
      echo "-----------------------------------------------"
      echo "Generating test suite for class [$class] (budget: ${class_budget}s)"
      echo ""
      ...
      cat $EVOSUITE_REPORT/statistics.csv >> $EVOSUITE_REPORT/statistics.${PROJECT}.${VERSION}b.${ID}.csv
...
#else
#  cd $D4J_EXPR/shell_scripts && sh extract_test.sh $PROJECT $VERSION $TOOL $ID $EVOSUITE_TEST
#fi
...
cd $D4J_EXPR && python3.6 carve_evosuite.py $PROJECT $VERSION -t $TOOL -i $ID
#cd $D4J_EXPR && python3.6 main.py $PROJECT $VERSION -t $TOOL -i $ID -b 20
