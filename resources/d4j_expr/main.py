...
def loop(project, version, tool, ts_id, selection_metric, budget=10, noise_prob=0.0, new=False, skip_broken=False, verbose=False):
  ...
  if os.path.exists(env.covmat_path) and os.path.exists(env.oravec_path):
    ...
    for test in failing_tests:
      coverage_path = os.path.join(env.coverage_dir, test + ".xml")
      #hits, covered_classes = parse_cobertura(coverage_path)
  ...
  if selection_metric != cEE:
    tests_path = tests_path.replace("tests", "tests-{}".format(selection_metric.__name__))
    method_ranks_path = method_ranks_path.replace("ranks", "ranks-{}".format(selection_metric.__name__))

  if os.path.exists(tests_path) and os.path.exists(method_ranks_path) and not new:
    ...
    tests, method_ranks = select(coverage_matrix, oracle_vector, buggy_methods,
                                 selection_metric, budget, noise_prob, skip_broken=skip_broken, verbose=verbose)
    tests.to_pickle(tests_path)
    method_ranks.to_pickle(method_ranks_path)
  return method_ranks

def select(coverage_matrix, oracle_vector, buggy_methods,
           selection_metric, budget=10, noise_prob=0.0, skip_broken=False, verbose=False):
  ...
  while True:
    ...
    y = (s_resvec['value'] == TestResult.SUCCESS.value).values.astype(np.uint8)
    ...
    for j, test in test_iterator:
      ...
      fitness = selection_metric(X, x, w=weights, target=torch.sum(X, dim=0) > 0, y=torch.tensor(y))
    ...
    #oracle = test_index[selected_test].oracle

    result_vector.iloc[selected_index]['value'] = response
    # if oracle == TestResult.UNKNOWN:
    #  print("=======================UNKNOwN===================")
...
if __name__ == "__main__":
  ...
  """
  loop(args.project, args.version, args.tool, args.id, eval(args.selection),
